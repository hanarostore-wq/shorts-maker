...
export function NaverBlogPanel(){
 ...
 const load=async()=>{const r=await fetch("/api/blog/naver",{cache:"no-store"});const j=await r.json();setItems(j.articles||[])};
 ...
 return <div className="flex flex-col gap-3">
   ...
   {items.map(a=><div key={a.id} className="mb-2 border border-zinc-800 bg-zinc-900/40 p-3">
    <div className="flex items-start justify-between gap-2"><div><div className="text-xs font-bold text-zinc-100">{a.title}</div><div className="mt-1 text-[10px] text-zinc-600">{a.id}</div></div><span className="text-[10px] text-cyan-300">{a.status.toUpperCase()}</span></div>
