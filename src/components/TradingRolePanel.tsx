...
function OrderbookOnlyPanel({ kind }: { kind: Kind }) {
  ...
  useEffect(() => {
    ...
    socket.onopen = () => socket.send(JSON.stringify([{ ticket: "moneyos-orderbook" }, { type: "orderbook", codes: ["KRW-BTC"], format: "DEFAULT" }]));
    socket.onmessage = async (event) => {
      try {
        const text = typeof event.data === "string" ? event.data : new TextDecoder().decode(await event.data.arrayBuffer());
        const data = JSON.parse(text) as { orderbook_units?: Book[] };
        if (active) { setRows(data.orderbook_units?.slice(0, 10) || []); setMessage("업비트 호가 실시간 수신 중"); }
      } catch { if (active) setMessage("[ORDERBOOK_PARSE_ERROR] 업비트 호가 응답 해석 실패"); }
    };
    socket.onerror = () => { if (active) setMessage("[ORDERBOOK_STREAM_ERROR] 업비트 호가 WebSocket 연결 실패"); };
