...
type Mode='paper'|'live';type Status={running:boolean;actualMode:Mode};
export function UpbitTerminalModal({mode,onClose}:{mode:Mode|null;onClose:()=>void}){
 ...
 <header className={styles.header}><div className={styles.heading}><strong id="upbit-workspace-title">{mode==='live'?'실전매매원':'모의매매원'} · 업비트</strong><span className={styles.paper}>운영본부 웹 매매</span><span role="status" className={styles.connection}>{failed?'화면 실행 확인 필요':mode&&status[mode]?'웹 매매 화면 준비됨':'매매 화면 준비 중…'}</span></div><button type="button" className={styles.close} onClick={onClose} aria-label="매매 화면 닫기">✕ 닫기</button></header>
