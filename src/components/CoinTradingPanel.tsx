...
function CandleChart({ candles = [] }: { candles?: Candle[] }) {
  const ordered = candles.slice(0, 60).reverse();
  if (!ordered.length) return <div className="h-40 p-8 text-center text-zinc-600">1분봉 데이터를 기다리는 중입니다</div>;
  const lows = ordered.map((c) => Number(c.low_price ?? c.trade_price));
  const highs = ordered.map((c) => Number(c.high_price ?? c.trade_price));
  const min = Math.min(...lows); const max = Math.max(...highs); const span = Math.max(max - min, 1);
...
export function CoinTradingPanel({ agent, showSimulation = true }: { agent: Agent; showSimulation?: boolean }) {
  ...
  const runSimulation = () => {
    const prices = (data?.candles || []).slice(0, 12).map((candle) => Number(candle.trade_price));
    if (!prices.length) { setMessage("모의 자동주문 테스트 오류: 먼저 1분봉 데이터를 받아야 합니다"); return; }
    setSimulation(prices.map((price, index) => ({ time: `${index + 1}틱`, price, action: index === 0 ? "관망" : index % 5 === 0 ? "매수 후보" : index % 7 === 0 ? "매도 후보" : "주문 차단", reason: index % 5 === 0 ? "SMA·RSI 조건 검토" : "리스크 관문 또는 전략 조건 미충족" })));
    setMessage("모의 자동주문 테스트 완료 · 실제 업비트 주문은 전송하지 않았습니다");
  };
