...
async function uploadImages(images) { if (!images?.length) return { count: 0, mode: 'none' }; const input = first(['input[type="file"][accept*="image"]', 'input[type="file"]']); if (!input) throw new Error('NAVER_IMAGE_INPUT_NOT_FOUND'); const files = []; for (const url of images) { const response = await fetch(url); if (!response.ok) throw new Error(`NAVER_IMAGE_FETCH_FAILED_${response.status}`); const blob = await response.blob(); files.push(new File([blob], `moneyos-${files.length + 1}.${(blob.type.split('/')[1] || 'jpg').replace('jpeg','jpg')}`, { type: blob.type })); } const transfer = new DataTransfer(); files.forEach((file) => transfer.items.add(file)); input.files = transfer.files; input.dispatchEvent(new Event('change', { bubbles: true })); await sleep(500); return { count: files.length, mode: 'file-input' }; }
